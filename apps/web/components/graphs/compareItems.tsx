...
export const CompareItems = ({
    ...
    const setServer = (active: boolean, server: CompareItem) => {
        ...
        if (active) {
            updatedSelectedServerIds = selectedItems.filter(
                (id) => id !== server.slug,
            )
        } else {
            ...
            <table className="w-full table-auto divide-y-2 divide-whiteBorder dark:divide-darkBorder">
                <thead>
                    <tr className="text-dark font-semibold dark:text-mainText dark:font-medium divide-whiteBorder">
                        ...
                        <th className="text-start p-4 max-tablet:hidden">
                            Mean
                        </th>
                        <th className="text-start p-4 max-tablet:hidden">
                            Max
                    ...
                    {compareItems.map((compareItem, index) => {
                        ...
                        return (
                            <tr
                                ...
                                key={index}
                                className={`fade hover:bg-whiteSelected hover:text-whiteMT dark:hover:bg-darkSelected dark:hover:text-mainText cursor-pointer ${
                                    active
                            ...
                            >
                                <td>
                                    <div
                                        className={`flex items-center justify-center w-2 h-14`}
                                        style={{
                                            backgroundColor: `${active ? compareItem.color : ""}`,
                                        }}
                                    />
                                </td>
                                ...
                                <td className="p-4 max-tablet:hidden">
                                    {compareItem.mean}
                                </td>
                                <td className="p-4 max-tablet:hidden">
                                    {compareItem.max}
