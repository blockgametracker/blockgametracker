...
export const PieChart = ({ data, enableArcLinkLabels }: Props) => {
    ...
    return (
        data.length !== 0 ? (
            <div className="flex flex-col w-full h-64 tablet:h-full justify-center">
                <ResponsivePie
                    data={data}
                    colors={(datum) => datum.data.color ?? ((resolvedTheme === "light") ? "#e7eaed":"#1f1f21")}
                    ...
                    defs={[
                        {
                            ...
                            type: "patternLines",
                            background: "inherit",
                            color: (resolvedTheme === "light") ? "rgba(204, 204, 204, 0.8)":"rgba(18, 18, 20, 0.8)",
                            rotation: -45,
                            lineWidth: 6,
                            spacing: 10,
                        },
